...
from strategy_lab.walk_forward_evaluation import bool_value, parse_dt, trade_key


def item(name: str) -> dict[str, object]:
    side = "long" if name.startswith("LONG_") else "short"
    return {
        "name": name,
        "threshold": "soft",
        "mode": name,
...
def main() -> int:
    ...
    for idx, value in enumerate(args):
        if value == "--candles" and idx + 1 < len(args):
            candles_path = Path(args[idx + 1])
        if value == "--out-dir" and idx + 1 < len(args):
            out_dir = Path(args[idx + 1])
    if candles_path is None:
        raise RuntimeError("--candles is required")

    rows = engine.read_candles(candles_path)
    btc = [row for row in rows if str(getattr(row, "symbol", "")).upper() == "BTCUSDT"]
    _REGIME_POINTS = build_regime_points(btc)
    if not _REGIME_POINTS:
        raise RuntimeError("no completed BTC regime points")
    ...
    if out_dir:
        ...
        if result_path.exists():
            ...
            payload["mode"] = "LONG_SHORT_REGIME_FAMILIES_V1"
            payload["promotion_allowed"] = False
            payload["external_holdout_touched"] = False
            payload["regime_points"] = len(_REGIME_POINTS)
            payload["limitations"] = [
                "Development-only screen on the fixed preregistered period.",
                "At most one passing leader may be frozen for one untouched external holdout.",
            ]
