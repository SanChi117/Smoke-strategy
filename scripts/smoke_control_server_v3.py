    ...
    bundle.update(
        {
            ...
            "candidates": core.query_rows(
                "SELECT * FROM candidates WHERE symbol=? ORDER BY entry_time DESC LIMIT 300", (symbol,)
            ),
            "trades": core.query_rows(
                "SELECT * FROM paper_trades WHERE symbol=? ORDER BY entry_time DESC LIMIT 300", (symbol,)
            ),
...
def market_overview() -> dict[str, Any]:
    ...
    ready = {
        ...
        for row in core.query_rows("SELECT symbol, COUNT(*) AS n FROM candidates WHERE final_status='READY' GROUP BY symbol")
    }
    opened = {
        str(row["symbol"]): int(row["n"])
        for row in core.query_rows("SELECT symbol, COUNT(*) AS n FROM paper_trades WHERE status='open' GROUP BY symbol")
    }
    items: list[dict[str, Any]] = []
    for symbol in universe:
        rows = aggregate_ohlcv(core.STORE.candles(symbol, core.SETTINGS.interval, 96), "15m")
        items.append(
            {
                "symbol": symbol,
                **latest_market_stats(rows),
        ...
        if parsed.path == "/api/terminal-capabilities":
            self._send(
                200,
                {
                    ...
                    "base_interval": core.SETTINGS.interval,
                    "features": [
                        "candles", "volume", "ema20", "ema50", "ema200", "crosshair",
                        "zoom_pan", "setup_markers", "entry_stop_target", "measurement",
                        "fullscreen", "persistent_layout",
                    ],
