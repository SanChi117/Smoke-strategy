...
def main() -> int:
    ...
    rows = [
        candle(at, 100, 103, 97, 101),
        candle(at + timedelta(minutes=5), 101, 105, 100, 104),
    ]
    exit_time, exit_price, reason, gross, funding, net = resolve_trade(
        ...
        stop=98,
        target=102,
        study_end=at + timedelta(minutes=10),
        funding={},
    )
    assert reason == "STOP"
    assert exit_price == 98
    assert exit_time == at + timedelta(minutes=5)
    ...
    candidate = TradeCandidate(
        ...
        stop=98,
        target=104,
        exit_time=at + timedelta(hours=1),
        exit_price=104,
        exit_reason="TARGET",
        gross_return_fraction=0.038,
        funding_return_fraction=0.0,
        net_return_fraction=0.038,
        structural_risk_fraction=0.02,
    ...
    assert summary["portfolio_max_drawdown_pct"] >= 0.0
