...
MTF architecture used by the tagged research path:
- 1D / 4H are market-context layers;
- the input interval, normally 15m, is the entry/setup layer;
- lower-timeframe confirmation is not faked here; it should be added as a
  separate 5m data source later instead of being confused with context.

This keeps the previous single-CSV pipeline compatible while fixing the main
logic bug: context must not be inferred only from the same narrow entry candle.
"""
...
def timeframe_bucket(time: datetime, hours: int) -> datetime:
    ...
    bucket_hour = (time.hour // hours) * hours
    return time.replace(hour=bucket_hour, minute=0, second=0, microsecond=0)
...
def resample_candles(rows: list[Candle], hours: int) -> list[Candle]:
    ...
    for bucket_rows in buckets.values():
        ...
        out.append(Candle(
            symbol=first.symbol,
            time=last.time,  # use last included candle time to avoid unfinished-bucket lookahead
            open=first.open,
...
def build_basic_context(
    ...
    for idx, candle in enumerate(sorted(rows, key=lambda c: c.time)):
        ...
        prev_fast_window = closes[-fast_len - 3:-3]
...
def combine_context(d1: dict[str, Any] | None, h4: dict[str, Any] | None, fallback_trend: str, fallback_dir: str, fallback_vol: str) -> tuple[str, str, str, str]:
    ...
    if "high" in vols:
        volatility = "high"
    elif vols and vols.issubset({"low", ""}) and (d1 or h4):
        volatility = "low"
    elif "normal" in vols:
        volatility = "normal"
    else:
...
def build_features(candles: Iterable[Candle], fast_len: int = 20, slow_len: int = 50, atr_len: int = 14, volume_len: int = 20, range_len: int = 50) -> list[MarketFeature]:
    ...
    for symbol, rows in by_symbol.items():
        ...
        h4_context = build_basic_context(resample_candles(rows, 4), fast_len=12, slow_len=30, atr_len=10, volume_len=12, range_len=30)
        d1_context = build_basic_context(resample_candles(rows, 24), fast_len=5, slow_len=20, atr_len=7, volume_len=10, range_len=20)
        ...
        for idx, candle in enumerate(rows):
            ...
            market_trend, market_direction, market_volatility, alignment = combine_context(
                d1,
                h4,
                fallback_trend=entry_trend,
                fallback_dir=entry_trend_direction,
                fallback_vol=entry_volatility,
            )
