...
class POIImbalanceEngineSmokeTest(unittest.TestCase):
    ...
    def test_bearish_origin_and_fvg_are_detected(self) -> None:
        ...
        self.assertTrue(any(item.event_type == "FVG_IMBALANCE" for item in snapshot.evidence))

    def test_origin_confirms_at_impulse_close_but_fvg_waits_for_third_close(self) -> None:
        rows = self.bullish_fixture()
        asof = rows[3].close_time
        snapshot = POIImbalanceEngine(self.config()).detect(rows, asof)
        self.assertTrue(any(
            item.source_type == POISourceType.ORIGIN_OF_DISPLACEMENT
            and item.confirmed_at == asof
            for item in snapshot.zones
        ))
        self.assertFalse(any(item.event_type == "FVG_IMBALANCE" for item in snapshot.evidence))
    ...
    def test_cluster_uses_diminishing_return_and_cap(self) -> None:
        ...
        result = cluster_evidence(ev, POIConfig(cluster_total_cap=30.0))[0]
    ...
    def test_overlapping_same_origin_zones_merge(self) -> None:
        a = zone(poi_id="a", low=99.0, high=100.0, origin_event_id="same")
        b = zone(poi_id="b", low=99.5, high=100.4, origin_event_id="same", source=POISourceType.FVG_IMBALANCE)
        merged = merge_composite_zones([a, b], POIConfig(merge_overlap_ratio=0.20))
    ...
    def test_opposite_direction_zones_do_not_merge(self) -> None:
        a = zone(poi_id="a", direction=Direction.LONG, origin_event_id="same")
        b = zone(poi_id="b", direction=Direction.SHORT, origin_event_id="same")
        self.assertEqual(len(merge_composite_zones([a, b])), 2)

    def test_lifecycle_active_without_touch(self) -> None:
        z = zone()
        rows = [bar(2, 101.0, 102.0, 100.5, 101.5)]
        result = evaluate_zone_lifecycle(z, rows, rows[-1].close_time)
    ...
    def test_external_location_and_liquidity_are_modifiers(self) -> None:
        z = zone()
        enriched = enrich_zone(z, location_score=90, liquidity_relation_score=85)
        self.assertGreater(enriched.quality_0_100, z.quality_0_100)
    ...
    def test_no_pnl_export_contains_no_outcome_fields(self) -> None:
        rows = self.bullish_fixture()
        snapshot = POIImbalanceEngine(self.config()).detect(rows, rows[-1].close_time)
        payload = snapshot_to_no_pnl_dict(snapshot)
